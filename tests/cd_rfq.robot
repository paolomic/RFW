...
Coh: Start New Session
    ...
    &{result}=          evaluate      test_cd_rfq.robot_run('do_ss_new_session','${arg}','${cfg_file}','new')  
    log                               result: ${result}
...
Coh: Starting Dialog
    ...
    [Timeout]           2 minutes
    ${arg}=             Set Variable  ${EMPTY}
    &{result}=          evaluate      test_cd_rfq.robot_run('do_ss_start_dialog','${arg}','${cfg_file}', 'hang') 
    ${info} =           Set Variable  ${result}[info]
    Should Be Equal As Strings        ${result}[status]     ok
...
Coh: Setting Init
    ...
    [Timeout]           2 minutes
    ${arg}=             Set Variable  ${EMPTY}
    &{result}=          evaluate      test_cd_rfq.robot_run('do_ss_setting_init','${arg}','${cfg_file}', 'hang') 
    ${info} =           Set Variable  ${result}[info]
    Should Be Equal As Strings        ${result}[status]     ok
...
Coh: Start Connection
    ...
    ${arg}=             Set Variable  ${EMPTY}
    &{result}=          evaluate      test_cd_rfq.robot_run('do_ss_start_connections','${arg}','${cfg_file}', 'hang') 
    log                               result: ${result}
    ${info} =           Set Variable  ${result}[info]
    Should Be Equal As Strings        ${result}[status]     ok
...
Coh: Reply Coherence - SellSide
    ...
    [Timeout]           2 minutes
    ${arg}=             Set Variable  ${EMPTY}
    &{result}=          evaluate      test_cd_rfq.robot_run('do_ss_reply','${arg}','${cfg_file}', 'hang') 
    ${info} =           Set Variable  ${result}[info]
    Should Be Equal As Strings        ${result}[status]     ok
