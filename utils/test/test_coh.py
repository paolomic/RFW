...
def handle_setting_init(wtop):
    ...
    #time.sleep(.25)
    #ud.dump_uia_tree(wtop)         # non c'e' lista popup
...
def handle_SecurityBrowser(wtop):
    ...
    page = uw.get_child(wtop, name='Security Browser.*', ctrl_type='Pane', use_re=1)
    print(f'page {page}')
    ...
    status_num = uw.get_child(page, name='No. of Rows: .*', ctrl_type='Text', deep=10, use_re=True)
    print(f'status_num {status_num}')
    assert status_num.window_text()=='No. of Rows: 1'       # UNIQUE 

    grid = status_num = uw.get_child(page, automation_id='59661', ctrl_type='Pane', deep=10, use_re=True)
    # hide
    uw.win_click(grid, mode='grid_row1')
    mouse.click('right')
    time.sleep(0.25)

    uw.popup_reply(wtop, 'New#Care Order')          # Il popup viene generato sotto level top - anche il sotto menu

def handle_NewCareOrder(env):
    dlg = uw.get_child(env.wtop, name="New Care Order.*", ctrl_type="Pane", use_re=1)
    adv = uw.get_child(dlg, automation_id='13652', ctrl_type="CheckBox", use_re=1)
...
def handle_order(order_id):
    ...
    time.sleep(.5)
    page = uw.get_child(env.wtop, name='Orders.*', ctrl_type='Pane', use_re=1, deep=2)
...
def run_session():
    env.init(COH_PATH, COH_TITLE_PATT)
    env.hang_app(mode='hang_or_launch')

    is_create = handle_start_dialog(env.wtop)
    time.sleep(2.5)                                                             # todo : Active Wait                     
    env.reload()                                                                # need reload UIA
...
def run_session_2():
    env.init(COH_PATH, COH_TITLE_PATT)
    env.hang_app(mode='hang')
    page = uw.get_child(env.wtop, name='Security Browser', ctrl_type='Pane', deep=3)
    grid = uw.get_child(page, name='StingrayGrid', deep=9)
...
def run_session_3(): 
    env.init(COH_PATH, COH_TITLE_PATT)
    env.hang_app(mode='hang_or_launch ')
    page = uw.get_child(env.wtop, name='Orders', ctrl_type='Pane', deep=3)
